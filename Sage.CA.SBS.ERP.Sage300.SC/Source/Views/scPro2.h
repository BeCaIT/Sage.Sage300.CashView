...
#if TEMPLATE_WARNING_LEVEL<=0 && !defined(SCPRO_ROTOID)
 #if defined LINUX_VER
  #warning ("The template disables some features when SCPRO_ROTOID has been removed")
  #warning ("from scPro.h. Please define it to activate these features")
 #else
  #pragma message ("The template disables some features when SCPRO_ROTOID has been removed")
  #pragma message ("from scPro.h. Please define it to activate these features")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_XXXFETCHFILTER) && (defined(DO_scProFetch) || defined(DO_scProFetchLock) || defined(DO_scProFilterFetch))
 #if defined LINUX_VER
  #warning ("Consider overriding xxxFetchFilter if you have previously overridden any of")
  #warning ("scProFetch, scProFetchLock or scProFilterFetch")
 #else
  #pragma message ("Consider overriding xxxFetchFilter if you have previously overridden any of")
  #pragma message ("scProFetch, scProFetchLock or scProFilterFetch")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_XXXREAD) && (defined(DO_scProRead) || defined(DO_scProReadLock))
 #if defined LINUX_VER
  #warning ("Consider overriding xxxRead if you have previously overridden any of")
  #warning ("scProRead or scProReadLock")
 #else
  #pragma message ("Consider overriding xxxRead if you have previously overridden any of")
  #pragma message ("scProRead or scProReadLock")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_INITBODYGEN) && (defined(DO_scProInit) || defined(DO_scProRecordClear) || defined(DO_scProRecordGenerate))
 #if defined LINUX_VER
  #warning ("Consider overriding InitBodyGen if you have previously overridden any of")
  #warning ("scProInit, scProRecordClear or scProRecordGenerate")
 #else
  #pragma message ("Consider overriding InitBodyGen if you have previously overridden any of")
  #pragma message ("scProInit, scProRecordClear or scProRecordGenerate")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && (!defined(DO_DELETEBODY) && !defined(DO_DELETEDETAILSBODY)) && (defined(DO_scProDelete) || defined(DO_scProFilterDelete))
 #if defined LINUX_VER
  #warning ("Consider overriding either DeleteBody or DeleteDetailsBody if you have previously")
  #warning ("overridden any of scProDelete or scProFilterDelete")
 #else
  #pragma message ("Consider overriding either DeleteBody or DeleteDetailsBody if you have previously")
  #pragma message ("overridden any of scProDelete or scProFilterDelete")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=0 && ((defined(DO_scProSecurity) && !defined(DO_scProInstanceSecurity)) || (!defined(DO_scProSecurity) && defined(DO_scProInstanceSecurity)))
 #if defined LINUX_VER
  #warning ("If either of scProSecurity or scProInstanceSecurity is overridden")
  #warning ("you should override the other")
 #else
  #pragma message ("If either of scProSecurity or scProInstanceSecurity is overridden")
...
// These used to be optional, now are required.
 ...
 #error DO_OLDPOST is no longer supported for non-revisioned views, and results in scProBasePost doing nothing. You need to remove it, plus change any callers that depended on this behaviour.
#endif
 ...
 #error DO_GETPERMISSIONS is no longer supported. You must override scProInstanceSecurity instead.
#endif
...
// The following provide simple error checking by view protocol
 ...
 #error View protocol must be explicitly selected
  ...
  #elif KEY_COUNT!=(SCPRO_KEY_COUNT-JOIN_KEY_COUNT+FAKE_KEY_COUNT)
   #error KEY_COUNT must be defined as (SCPRO_KEY_COUNT-JOIN_KEY_COUNT+FAKE_KEY_COUNT)
  #endif
  ...
  #error Under C++ you must derive VEXTRA in scPro3.h from VBASE
 #endif
...
struct tagV {
   ...
   BOOL bVerifying; // inside scProVerify routine
   ...
   WORD wKey; // Current key as passed to scProOrder
   ...
   LPVOID OpenExtra; // copy of extra data received by scProInstanceOpen
   ...
   LONG rotoID; // My own rotoID; needs SCPRO_ROTOID to be defined to be meaningful
...
#if !defined(__cplusplus)
 #if defined(DO_VIEWHANDLE)
   VEXTRA extra; // Programmer-defined view handle extensions, defined in scPro1.h
 #endif
...
// definition in the scPro1.c file must be done.
...
// initialized in the scPro1.c file.
...
/////////////////////////////////////////////////////////////////////////////
// Declarations for generally available subroutines found in scPro.c

// The fileXXXX routines are not always defined by the template. For stub
// views they appear in scPro1.c, and must be specified by the programmer.
...
VIEWCALL scProFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scProFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
VIEWCALL scProFields            (LPVOID lpView, LPSHORT lFields);
VIEWCALL scProFilterCount       (LPV view, LPSTR filter, A4W_FLAGS flags, LP_A4W_SIZE count);
...
VIEWCALL scProInstanceFieldList (LPV view, A4W_FLAGS flags, A4W_SIZE count, PLONG fields);
VIEWCALL scProInstanceNotify    (LPV view, A4W_NOTIFY_PROC notify, LPVOID context, A4W_FLAGS flags);
VIEWCALL scProInstanceOpen      (LPRVH rvh, HPIB PIB, WORD link, A4W_FLAGS flags, LPVOID extra, A4W_SIZE size, LPV FAR * view);
...
VIEWCALL scProOpen              (HPIB hPIB, WORD wLinkNo, LPVOID FAR * lpView);
...
VIEWCALL scProRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
...
VIEWCALL scProViewTableCount    (INT roto, LPSHORT tables);
VIEWCALL scProViewTables        (INT roto, LPSTR tables, INT size);
...
VIEWCALL scProBaseFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scProBaseFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
...
VIEWCALL scProBaseRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
...
VIEWCALL scProExtendedBaseAttribs           (LPVOID lpView, LONG lField, LPLONG lpAttributes);
VIEWCALL scProExtendedBaseBlkGet            (LPVOID lpView, INT lFields, LPLONG lFieldList, LPSHORT lpnSizeList, LPSTR lpsBuffer);
VIEWCALL scProExtendedBaseBlkPut            (LPVOID lpView, INT lFields, LPLONG lFieldList, LPSHORT lpnSizeList, LPSTR lpsBuffer, BOOL bVerify);
VIEWCALL scProExtendedBaseBrowse            (LPVOID lpView, LPSTR lpsExpression, BOOL bAscending);
VIEWCALL scProExtendedBaseCancel            (LPVOID lpView);
VIEWCALL scProExtendedBaseClose             (LPVOID lpView);
VIEWCALL scProExtendedBaseCmpNames          (INT roto, LPSTR lpsCompositeNames, LPSHORT lpnCount);
VIEWCALL scProExtendedBaseCompose           (LPVOID lpView, LPVIEWDEF lpvdefComposites);
VIEWCALL scProExtendedBaseCreate            (HPIB hPIB, WORD wLinkNo);
VIEWCALL scProExtendedBaseDelete            (LPVOID lpView);
VIEWCALL scProExtendedBaseDirty             (LPVOID lpView, LPBOOL bChanged);
VIEWCALL scProExtendedBaseDrop              (HPIB hPIB, WORD wLinkNo);
VIEWCALL scProExtendedBaseExists            (LPVOID lpView, LPBOOL bExists);
VIEWCALL scProExtendedBaseFetch             (LPVOID lpView);
VIEWCALL scProExtendedBaseFetchLock         (LPVOID lpView);
VIEWCALL scProExtendedBaseField             (LPVOID lpView, INT nFldNumber, LPSTR lpsFldName, LPLONG lField);
VIEWCALL scProExtendedBaseFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scProExtendedBaseFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
VIEWCALL scProExtendedBaseFields            (LPVOID lpView, LPSHORT lFields);
VIEWCALL scProExtendedBaseFilterCount       (LPV view, LPSTR filter, A4W_FLAGS flags, LP_A4W_SIZE count);
VIEWCALL scProExtendedBaseFilterDelete      (LPV view, LPSTR filter, A4W_FLAGS flags);
VIEWCALL scProExtendedBaseFilterFetch       (LPV view, A4W_FLAGS flags);
VIEWCALL scProExtendedBaseFilterSelect      (LPV view, INT key, LPSTR filter, A4W_FLAGS flags);
VIEWCALL scProExtendedBaseGet               (LPVOID lpView, LONG lField, LPSTR buffer, INT nSize);
VIEWCALL scProExtendedBaseInit              (LPVOID lpView);
VIEWCALL scProExtendedBaseInsert            (LPVOID lpView);
...
VIEWCALL scProExtendedBaseInstanceProtocol  (LPV view, A4W_FLAGS flags, LP_A4W_FLAGS protocols, LP_A4W_SIZE count);
VIEWCALL scProExtendedBaseInstanceSecurity  (LPV view, LP_A4W_FLAGS flags);
VIEWCALL scProExtendedBaseKey               (LPVOID lpView, INT nKeyNo, LPSTR sName, LPSHORT lFields, LPLONG lplKeyFields);
VIEWCALL scProExtendedBaseKeys              (LPVOID lpView, LPSHORT lpnKeys);
VIEWCALL scProExtendedBaseName              (LPVOID lpView, LONG lField, LPSTR lpsName, LPSTR lpsDesc);
VIEWCALL scProExtendedBaseOpen              (HPIB hPIB, WORD wLinkNo, LPVOID FAR * lpView);
VIEWCALL scProExtendedBaseOrder             (LPVOID lpView, INT nKey);
VIEWCALL scProExtendedBasePost              (LPVOID lpView);
VIEWCALL scProExtendedBasePresents          (LPVOID lpView, LONG lField, LPSTR lpsMask, LPSHORT lpnList, LPSHORT lpnCount, LPVOID lpBuffer, INT nSize);
VIEWCALL scProExtendedBaseProcess           (LPVOID lpView);
VIEWCALL scProExtendedBasePut               (LPVOID lpView, LONG lField, LPSTR lpsBuffer, INT nSize, BOOL bVerify);
VIEWCALL scProExtendedBaseRead              (LPVOID lpView);
VIEWCALL scProExtendedBaseReadLock          (LPVOID lpView);
VIEWCALL scProExtendedBaseRecordClear       (LPV view);
VIEWCALL scProExtendedBaseRecordGenerate    (LPV view, A4W_FLAGS flags);
VIEWCALL scProExtendedBaseRevisionCancel    (LPV view, INT level);
VIEWCALL scProExtendedBaseRevisionExists    (LPV view, INT level, LP_A4W_FLAGS flags);
VIEWCALL scProExtendedBaseRevisionPost      (LPV view, INT level);
VIEWCALL scProExtendedBaseRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
VIEWCALL scProExtendedBaseRevisionUnposted  (LPV view, INT level, LP_A4W_FLAGS flags);
VIEWCALL scProExtendedBaseSecurity          (HPIB hPIB, WORD wLinkNo, LPLONG lplPermissions);
VIEWCALL scProExtendedBaseTableEmpty        (LPV view);
VIEWCALL scProExtendedBaseType              (LPVOID lpView, LONG lField, LPTYPE lptype);
VIEWCALL scProExtendedBaseUnLock            (LPVOID lpView);
VIEWCALL scProExtendedBaseUpdate            (LPVOID lpView);
VIEWCALL scProExtendedBaseVerify            (LPVOID lpView);
...
VIEWCALL viewOldVerify            (LPVOID rvh, LPVOID view);
 ...
 #if defined(__scPro_c) || DO_EXTENDEDERRORS>NO_EXTENDEDERRORS
