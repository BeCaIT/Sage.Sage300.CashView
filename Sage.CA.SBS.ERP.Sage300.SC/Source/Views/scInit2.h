...
#if !defined(__scInit2_h)
// Ensures header is only included once.
#define __scInit2_h        1
...
#if TEMPLATE_WARNING_LEVEL<=0 && !defined(SCINIT_ROTOID)
 #if defined LINUX_VER
  #warning ("The template disables some features when SCINIT_ROTOID has been removed")
  #warning ("from scInit.h. Please define it to activate these features")
 #else
  #pragma message ("The template disables some features when SCINIT_ROTOID has been removed")
  #pragma message ("from scInit.h. Please define it to activate these features")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_XXXFETCHFILTER) && (defined(DO_scInitFetch) || defined(DO_scInitFetchLock) || defined(DO_scInitFilterFetch))
 #if defined LINUX_VER
  #warning ("Consider overriding xxxFetchFilter if you have previously overridden any of")
  #warning ("scInitFetch, scInitFetchLock or scInitFilterFetch")
 #else
  #pragma message ("Consider overriding xxxFetchFilter if you have previously overridden any of")
  #pragma message ("scInitFetch, scInitFetchLock or scInitFilterFetch")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_XXXREAD) && (defined(DO_scInitRead) || defined(DO_scInitReadLock))
 #if defined LINUX_VER
  #warning ("Consider overriding xxxRead if you have previously overridden any of")
  #warning ("scInitRead or scInitReadLock")
 #else
  #pragma message ("Consider overriding xxxRead if you have previously overridden any of")
  #pragma message ("scInitRead or scInitReadLock")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_INITBODYGEN) && (defined(DO_scInitInit) || defined(DO_scInitRecordClear) || defined(DO_scInitRecordGenerate))
 #if defined LINUX_VER
  #warning ("Consider overriding InitBodyGen if you have previously overridden any of")
  #warning ("scInitInit, scInitRecordClear or scInitRecordGenerate")
 #else
  #pragma message ("Consider overriding InitBodyGen if you have previously overridden any of")
  #pragma message ("scInitInit, scInitRecordClear or scInitRecordGenerate")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && (!defined(DO_DELETEBODY) && !defined(DO_DELETEDETAILSBODY)) && (defined(DO_scInitDelete) || defined(DO_scInitFilterDelete))
 #if defined LINUX_VER
  #warning ("Consider overriding either DeleteBody or DeleteDetailsBody if you have previously")
  #warning ("overridden any of scInitDelete or scInitFilterDelete")
 #else
  #pragma message ("Consider overriding either DeleteBody or DeleteDetailsBody if you have previously")
  #pragma message ("overridden any of scInitDelete or scInitFilterDelete")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=0 && ((defined(DO_scInitSecurity) && !defined(DO_scInitInstanceSecurity)) || (!defined(DO_scInitSecurity) && defined(DO_scInitInstanceSecurity)))
 #if defined LINUX_VER
  #warning ("If either of scInitSecurity or scInitInstanceSecurity is overridden")
  #warning ("you should override the other")
 #else
  #pragma message ("If either of scInitSecurity or scInitInstanceSecurity is overridden")
...
// These used to be optional, now are required.
 ...
 #error DO_OLDPOST is no longer supported for non-revisioned views, and results in scInitBasePost doing nothing. You need to remove it, plus change any callers that depended on this behaviour.
#endif
 ...
 #error DO_GETPERMISSIONS is no longer supported. You must override scInitInstanceSecurity instead.
#endif
...
// The following provide simple error checking by view protocol
 ...
 #error View protocol must be explicitly selected
  ...
  #elif KEY_COUNT!=(SCINIT_KEY_COUNT-JOIN_KEY_COUNT+FAKE_KEY_COUNT)
   #error KEY_COUNT must be defined as (SCINIT_KEY_COUNT-JOIN_KEY_COUNT+FAKE_KEY_COUNT)
  #endif
  ...
  #error Under C++ you must derive VEXTRA in scInit3.h from VBASE
 #endif
...
struct tagV {
   ...
   BOOL bVerifying; // inside scInitVerify routine
   ...
   WORD wKey; // Current key as passed to scInitOrder
   ...
   LPVOID OpenExtra; // copy of extra data received by scInitInstanceOpen
   ...
   LONG rotoID; // My own rotoID; needs SCINIT_ROTOID to be defined to be meaningful
...
#if !defined(__cplusplus)
 #if defined(DO_VIEWHANDLE)
   VEXTRA extra; // Programmer-defined view handle extensions, defined in scInit1.h
 #endif
...
// definition in the scInit1.c file must be done.
...
// initialized in the scInit1.c file.
...
/////////////////////////////////////////////////////////////////////////////
// Declarations for generally available subroutines found in scInit.c

// The fileXXXX routines are not always defined by the template. For stub
// views they appear in scInit1.c, and must be specified by the programmer.
...
VIEWCALL scInitFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scInitFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
VIEWCALL scInitFields            (LPVOID lpView, LPSHORT lFields);
VIEWCALL scInitFilterCount       (LPV view, LPSTR filter, A4W_FLAGS flags, LP_A4W_SIZE count);
...
VIEWCALL scInitInstanceFieldList (LPV view, A4W_FLAGS flags, A4W_SIZE count, PLONG fields);
VIEWCALL scInitInstanceNotify    (LPV view, A4W_NOTIFY_PROC notify, LPVOID context, A4W_FLAGS flags);
VIEWCALL scInitInstanceOpen      (LPRVH rvh, HPIB PIB, WORD link, A4W_FLAGS flags, LPVOID extra, A4W_SIZE size, LPV FAR * view);
...
VIEWCALL scInitOpen              (HPIB hPIB, WORD wLinkNo, LPVOID FAR * lpView);
...
VIEWCALL scInitRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
...
VIEWCALL scInitViewTableCount    (INT roto, LPSHORT tables);
VIEWCALL scInitViewTables        (INT roto, LPSTR tables, INT size);
...
VIEWCALL scInitBaseFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scInitBaseFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
...
VIEWCALL scInitBaseRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
...
VIEWCALL scInitExtendedBaseAttribs           (LPVOID lpView, LONG lField, LPLONG lpAttributes);
VIEWCALL scInitExtendedBaseBlkGet            (LPVOID lpView, INT lFields, LPLONG lFieldList, LPSHORT lpnSizeList, LPSTR lpsBuffer);
VIEWCALL scInitExtendedBaseBlkPut            (LPVOID lpView, INT lFields, LPLONG lFieldList, LPSHORT lpnSizeList, LPSTR lpsBuffer, BOOL bVerify);
VIEWCALL scInitExtendedBaseBrowse            (LPVOID lpView, LPSTR lpsExpression, BOOL bAscending);
VIEWCALL scInitExtendedBaseCancel            (LPVOID lpView);
VIEWCALL scInitExtendedBaseClose             (LPVOID lpView);
VIEWCALL scInitExtendedBaseCmpNames          (INT roto, LPSTR lpsCompositeNames, LPSHORT lpnCount);
VIEWCALL scInitExtendedBaseCompose           (LPVOID lpView, LPVIEWDEF lpvdefComposites);
VIEWCALL scInitExtendedBaseCreate            (HPIB hPIB, WORD wLinkNo);
VIEWCALL scInitExtendedBaseDelete            (LPVOID lpView);
VIEWCALL scInitExtendedBaseDirty             (LPVOID lpView, LPBOOL bChanged);
VIEWCALL scInitExtendedBaseDrop              (HPIB hPIB, WORD wLinkNo);
VIEWCALL scInitExtendedBaseExists            (LPVOID lpView, LPBOOL bExists);
VIEWCALL scInitExtendedBaseFetch             (LPVOID lpView);
VIEWCALL scInitExtendedBaseFetchLock         (LPVOID lpView);
VIEWCALL scInitExtendedBaseField             (LPVOID lpView, INT nFldNumber, LPSTR lpsFldName, LPLONG lField);
VIEWCALL scInitExtendedBaseFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scInitExtendedBaseFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
VIEWCALL scInitExtendedBaseFields            (LPVOID lpView, LPSHORT lFields);
VIEWCALL scInitExtendedBaseFilterCount       (LPV view, LPSTR filter, A4W_FLAGS flags, LP_A4W_SIZE count);
VIEWCALL scInitExtendedBaseFilterDelete      (LPV view, LPSTR filter, A4W_FLAGS flags);
VIEWCALL scInitExtendedBaseFilterFetch       (LPV view, A4W_FLAGS flags);
VIEWCALL scInitExtendedBaseFilterSelect      (LPV view, INT key, LPSTR filter, A4W_FLAGS flags);
VIEWCALL scInitExtendedBaseGet               (LPVOID lpView, LONG lField, LPSTR buffer, INT nSize);
VIEWCALL scInitExtendedBaseInit              (LPVOID lpView);
VIEWCALL scInitExtendedBaseInsert            (LPVOID lpView);
...
VIEWCALL scInitExtendedBaseInstanceProtocol  (LPV view, A4W_FLAGS flags, LP_A4W_FLAGS protocols, LP_A4W_SIZE count);
VIEWCALL scInitExtendedBaseInstanceSecurity  (LPV view, LP_A4W_FLAGS flags);
VIEWCALL scInitExtendedBaseKey               (LPVOID lpView, INT nKeyNo, LPSTR sName, LPSHORT lFields, LPLONG lplKeyFields);
VIEWCALL scInitExtendedBaseKeys              (LPVOID lpView, LPSHORT lpnKeys);
VIEWCALL scInitExtendedBaseName              (LPVOID lpView, LONG lField, LPSTR lpsName, LPSTR lpsDesc);
VIEWCALL scInitExtendedBaseOpen              (HPIB hPIB, WORD wLinkNo, LPVOID FAR * lpView);
VIEWCALL scInitExtendedBaseOrder             (LPVOID lpView, INT nKey);
VIEWCALL scInitExtendedBasePost              (LPVOID lpView);
VIEWCALL scInitExtendedBasePresents          (LPVOID lpView, LONG lField, LPSTR lpsMask, LPSHORT lpnList, LPSHORT lpnCount, LPVOID lpBuffer, INT nSize);
VIEWCALL scInitExtendedBaseProcess           (LPVOID lpView);
VIEWCALL scInitExtendedBasePut               (LPVOID lpView, LONG lField, LPSTR lpsBuffer, INT nSize, BOOL bVerify);
VIEWCALL scInitExtendedBaseRead              (LPVOID lpView);
VIEWCALL scInitExtendedBaseReadLock          (LPVOID lpView);
VIEWCALL scInitExtendedBaseRecordClear       (LPV view);
VIEWCALL scInitExtendedBaseRecordGenerate    (LPV view, A4W_FLAGS flags);
VIEWCALL scInitExtendedBaseRevisionCancel    (LPV view, INT level);
VIEWCALL scInitExtendedBaseRevisionExists    (LPV view, INT level, LP_A4W_FLAGS flags);
VIEWCALL scInitExtendedBaseRevisionPost      (LPV view, INT level);
VIEWCALL scInitExtendedBaseRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
VIEWCALL scInitExtendedBaseRevisionUnposted  (LPV view, INT level, LP_A4W_FLAGS flags);
VIEWCALL scInitExtendedBaseSecurity          (HPIB hPIB, WORD wLinkNo, LPLONG lplPermissions);
VIEWCALL scInitExtendedBaseTableEmpty        (LPV view);
VIEWCALL scInitExtendedBaseType              (LPVOID lpView, LONG lField, LPTYPE lptype);
VIEWCALL scInitExtendedBaseUnLock            (LPVOID lpView);
VIEWCALL scInitExtendedBaseUpdate            (LPVOID lpView);
VIEWCALL scInitExtendedBaseVerify            (LPVOID lpView);
...
VIEWCALL viewOldVerify            (LPVOID rvh, LPVOID view);
 ...
 #if defined(__scInit_c) || DO_EXTENDEDERRORS>NO_EXTENDEDERRORS
