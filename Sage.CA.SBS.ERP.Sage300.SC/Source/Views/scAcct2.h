...
#if !defined(__scAcct2_h)
// Ensures header is only included once.
#define __scAcct2_h        1
...
#if TEMPLATE_WARNING_LEVEL<=0 && !defined(SCACCT_ROTOID)
 #if defined LINUX_VER
  #warning ("The template disables some features when SCACCT_ROTOID has been removed")
  #warning ("from scAcct.h. Please define it to activate these features")
 #else
  #pragma message ("The template disables some features when SCACCT_ROTOID has been removed")
  #pragma message ("from scAcct.h. Please define it to activate these features")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_XXXFETCHFILTER) && (defined(DO_scAcctFetch) || defined(DO_scAcctFetchLock) || defined(DO_scAcctFilterFetch))
 #if defined LINUX_VER
  #warning ("Consider overriding xxxFetchFilter if you have previously overridden any of")
  #warning ("scAcctFetch, scAcctFetchLock or scAcctFilterFetch")
 #else
  #pragma message ("Consider overriding xxxFetchFilter if you have previously overridden any of")
  #pragma message ("scAcctFetch, scAcctFetchLock or scAcctFilterFetch")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_XXXREAD) && (defined(DO_scAcctRead) || defined(DO_scAcctReadLock))
 #if defined LINUX_VER
  #warning ("Consider overriding xxxRead if you have previously overridden any of")
  #warning ("scAcctRead or scAcctReadLock")
 #else
  #pragma message ("Consider overriding xxxRead if you have previously overridden any of")
  #pragma message ("scAcctRead or scAcctReadLock")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_INITBODYGEN) && (defined(DO_scAcctInit) || defined(DO_scAcctRecordClear) || defined(DO_scAcctRecordGenerate))
 #if defined LINUX_VER
  #warning ("Consider overriding InitBodyGen if you have previously overridden any of")
  #warning ("scAcctInit, scAcctRecordClear or scAcctRecordGenerate")
 #else
  #pragma message ("Consider overriding InitBodyGen if you have previously overridden any of")
  #pragma message ("scAcctInit, scAcctRecordClear or scAcctRecordGenerate")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && (!defined(DO_DELETEBODY) && !defined(DO_DELETEDETAILSBODY)) && (defined(DO_scAcctDelete) || defined(DO_scAcctFilterDelete))
 #if defined LINUX_VER
  #warning ("Consider overriding either DeleteBody or DeleteDetailsBody if you have previously")
  #warning ("overridden any of scAcctDelete or scAcctFilterDelete")
 #else
  #pragma message ("Consider overriding either DeleteBody or DeleteDetailsBody if you have previously")
  #pragma message ("overridden any of scAcctDelete or scAcctFilterDelete")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=0 && ((defined(DO_scAcctSecurity) && !defined(DO_scAcctInstanceSecurity)) || (!defined(DO_scAcctSecurity) && defined(DO_scAcctInstanceSecurity)))
 #if defined LINUX_VER
  #warning ("If either of scAcctSecurity or scAcctInstanceSecurity is overridden")
  #warning ("you should override the other")
 #else
  #pragma message ("If either of scAcctSecurity or scAcctInstanceSecurity is overridden")
...
// These used to be optional, now are required.
 ...
 #error DO_OLDPOST is no longer supported for non-revisioned views, and results in scAcctBasePost doing nothing. You need to remove it, plus change any callers that depended on this behaviour.
#endif
 ...
 #error DO_GETPERMISSIONS is no longer supported. You must override scAcctInstanceSecurity instead.
#endif
...
// The following provide simple error checking by view protocol
 ...
 #error View protocol must be explicitly selected
  ...
  #elif KEY_COUNT!=(SCACCT_KEY_COUNT-JOIN_KEY_COUNT+FAKE_KEY_COUNT)
   #error KEY_COUNT must be defined as (SCACCT_KEY_COUNT-JOIN_KEY_COUNT+FAKE_KEY_COUNT)
  #endif
  ...
  #error Under C++ you must derive VEXTRA in scAcct3.h from VBASE
 #endif
...
struct tagV {
   ...
   BOOL bVerifying; // inside scAcctVerify routine
   ...
   WORD wKey; // Current key as passed to scAcctOrder
   ...
   LPVOID OpenExtra; // copy of extra data received by scAcctInstanceOpen
   ...
   LONG rotoID; // My own rotoID; needs SCACCT_ROTOID to be defined to be meaningful
...
#if !defined(__cplusplus)
 #if defined(DO_VIEWHANDLE)
   VEXTRA extra; // Programmer-defined view handle extensions, defined in scAcct1.h
 #endif
...
// definition in the scAcct1.c file must be done.
...
// initialized in the scAcct1.c file.
...
/////////////////////////////////////////////////////////////////////////////
// Declarations for generally available subroutines found in scAcct.c

// The fileXXXX routines are not always defined by the template. For stub
// views they appear in scAcct1.c, and must be specified by the programmer.
...
VIEWCALL scAcctFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scAcctFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
VIEWCALL scAcctFields            (LPVOID lpView, LPSHORT lFields);
VIEWCALL scAcctFilterCount       (LPV view, LPSTR filter, A4W_FLAGS flags, LP_A4W_SIZE count);
...
VIEWCALL scAcctInstanceFieldList (LPV view, A4W_FLAGS flags, A4W_SIZE count, PLONG fields);
VIEWCALL scAcctInstanceNotify    (LPV view, A4W_NOTIFY_PROC notify, LPVOID context, A4W_FLAGS flags);
VIEWCALL scAcctInstanceOpen      (LPRVH rvh, HPIB PIB, WORD link, A4W_FLAGS flags, LPVOID extra, A4W_SIZE size, LPV FAR * view);
...
VIEWCALL scAcctOpen              (HPIB hPIB, WORD wLinkNo, LPVOID FAR * lpView);
...
VIEWCALL scAcctRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
...
VIEWCALL scAcctViewTableCount    (INT roto, LPSHORT tables);
VIEWCALL scAcctViewTables        (INT roto, LPSTR tables, INT size);
...
VIEWCALL scAcctBaseFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scAcctBaseFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
...
VIEWCALL scAcctBaseRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
...
VIEWCALL scAcctExtendedBaseAttribs           (LPVOID lpView, LONG lField, LPLONG lpAttributes);
VIEWCALL scAcctExtendedBaseBlkGet            (LPVOID lpView, INT lFields, LPLONG lFieldList, LPSHORT lpnSizeList, LPSTR lpsBuffer);
VIEWCALL scAcctExtendedBaseBlkPut            (LPVOID lpView, INT lFields, LPLONG lFieldList, LPSHORT lpnSizeList, LPSTR lpsBuffer, BOOL bVerify);
VIEWCALL scAcctExtendedBaseBrowse            (LPVOID lpView, LPSTR lpsExpression, BOOL bAscending);
VIEWCALL scAcctExtendedBaseCancel            (LPVOID lpView);
VIEWCALL scAcctExtendedBaseClose             (LPVOID lpView);
VIEWCALL scAcctExtendedBaseCmpNames          (INT roto, LPSTR lpsCompositeNames, LPSHORT lpnCount);
VIEWCALL scAcctExtendedBaseCompose           (LPVOID lpView, LPVIEWDEF lpvdefComposites);
VIEWCALL scAcctExtendedBaseCreate            (HPIB hPIB, WORD wLinkNo);
VIEWCALL scAcctExtendedBaseDelete            (LPVOID lpView);
VIEWCALL scAcctExtendedBaseDirty             (LPVOID lpView, LPBOOL bChanged);
VIEWCALL scAcctExtendedBaseDrop              (HPIB hPIB, WORD wLinkNo);
VIEWCALL scAcctExtendedBaseExists            (LPVOID lpView, LPBOOL bExists);
VIEWCALL scAcctExtendedBaseFetch             (LPVOID lpView);
VIEWCALL scAcctExtendedBaseFetchLock         (LPVOID lpView);
VIEWCALL scAcctExtendedBaseField             (LPVOID lpView, INT nFldNumber, LPSTR lpsFldName, LPLONG lField);
VIEWCALL scAcctExtendedBaseFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scAcctExtendedBaseFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
VIEWCALL scAcctExtendedBaseFields            (LPVOID lpView, LPSHORT lFields);
VIEWCALL scAcctExtendedBaseFilterCount       (LPV view, LPSTR filter, A4W_FLAGS flags, LP_A4W_SIZE count);
VIEWCALL scAcctExtendedBaseFilterDelete      (LPV view, LPSTR filter, A4W_FLAGS flags);
VIEWCALL scAcctExtendedBaseFilterFetch       (LPV view, A4W_FLAGS flags);
VIEWCALL scAcctExtendedBaseFilterSelect      (LPV view, INT key, LPSTR filter, A4W_FLAGS flags);
VIEWCALL scAcctExtendedBaseGet               (LPVOID lpView, LONG lField, LPSTR buffer, INT nSize);
VIEWCALL scAcctExtendedBaseInit              (LPVOID lpView);
VIEWCALL scAcctExtendedBaseInsert            (LPVOID lpView);
...
VIEWCALL scAcctExtendedBaseInstanceProtocol  (LPV view, A4W_FLAGS flags, LP_A4W_FLAGS protocols, LP_A4W_SIZE count);
VIEWCALL scAcctExtendedBaseInstanceSecurity  (LPV view, LP_A4W_FLAGS flags);
VIEWCALL scAcctExtendedBaseKey               (LPVOID lpView, INT nKeyNo, LPSTR sName, LPSHORT lFields, LPLONG lplKeyFields);
VIEWCALL scAcctExtendedBaseKeys              (LPVOID lpView, LPSHORT lpnKeys);
VIEWCALL scAcctExtendedBaseName              (LPVOID lpView, LONG lField, LPSTR lpsName, LPSTR lpsDesc);
VIEWCALL scAcctExtendedBaseOpen              (HPIB hPIB, WORD wLinkNo, LPVOID FAR * lpView);
VIEWCALL scAcctExtendedBaseOrder             (LPVOID lpView, INT nKey);
VIEWCALL scAcctExtendedBasePost              (LPVOID lpView);
VIEWCALL scAcctExtendedBasePresents          (LPVOID lpView, LONG lField, LPSTR lpsMask, LPSHORT lpnList, LPSHORT lpnCount, LPVOID lpBuffer, INT nSize);
VIEWCALL scAcctExtendedBaseProcess           (LPVOID lpView);
VIEWCALL scAcctExtendedBasePut               (LPVOID lpView, LONG lField, LPSTR lpsBuffer, INT nSize, BOOL bVerify);
VIEWCALL scAcctExtendedBaseRead              (LPVOID lpView);
VIEWCALL scAcctExtendedBaseReadLock          (LPVOID lpView);
VIEWCALL scAcctExtendedBaseRecordClear       (LPV view);
VIEWCALL scAcctExtendedBaseRecordGenerate    (LPV view, A4W_FLAGS flags);
VIEWCALL scAcctExtendedBaseRevisionCancel    (LPV view, INT level);
VIEWCALL scAcctExtendedBaseRevisionExists    (LPV view, INT level, LP_A4W_FLAGS flags);
VIEWCALL scAcctExtendedBaseRevisionPost      (LPV view, INT level);
VIEWCALL scAcctExtendedBaseRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
VIEWCALL scAcctExtendedBaseRevisionUnposted  (LPV view, INT level, LP_A4W_FLAGS flags);
VIEWCALL scAcctExtendedBaseSecurity          (HPIB hPIB, WORD wLinkNo, LPLONG lplPermissions);
VIEWCALL scAcctExtendedBaseTableEmpty        (LPV view);
VIEWCALL scAcctExtendedBaseType              (LPVOID lpView, LONG lField, LPTYPE lptype);
VIEWCALL scAcctExtendedBaseUnLock            (LPVOID lpView);
VIEWCALL scAcctExtendedBaseUpdate            (LPVOID lpView);
VIEWCALL scAcctExtendedBaseVerify            (LPVOID lpView);
...
VIEWCALL viewOldVerify            (LPVOID rvh, LPVOID view);
 ...
 #if defined(__scAcct_c) || DO_EXTENDEDERRORS>NO_EXTENDEDERRORS
