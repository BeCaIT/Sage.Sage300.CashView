...
#if TEMPLATE_WARNING_LEVEL<=0 && !defined(SCBAL_ROTOID)
 #if defined LINUX_VER
  #warning ("The template disables some features when SCBAL_ROTOID has been removed")
  #warning ("from scBal.h. Please define it to activate these features")
 #else
  #pragma message ("The template disables some features when SCBAL_ROTOID has been removed")
  #pragma message ("from scBal.h. Please define it to activate these features")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_XXXFETCHFILTER) && (defined(DO_scBalFetch) || defined(DO_scBalFetchLock) || defined(DO_scBalFilterFetch))
 #if defined LINUX_VER
  #warning ("Consider overriding xxxFetchFilter if you have previously overridden any of")
  #warning ("scBalFetch, scBalFetchLock or scBalFilterFetch")
 #else
  #pragma message ("Consider overriding xxxFetchFilter if you have previously overridden any of")
  #pragma message ("scBalFetch, scBalFetchLock or scBalFilterFetch")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_XXXREAD) && (defined(DO_scBalRead) || defined(DO_scBalReadLock))
 #if defined LINUX_VER
  #warning ("Consider overriding xxxRead if you have previously overridden any of")
  #warning ("scBalRead or scBalReadLock")
 #else
  #pragma message ("Consider overriding xxxRead if you have previously overridden any of")
  #pragma message ("scBalRead or scBalReadLock")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_INITBODYGEN) && (defined(DO_scBalInit) || defined(DO_scBalRecordClear) || defined(DO_scBalRecordGenerate))
 #if defined LINUX_VER
  #warning ("Consider overriding InitBodyGen if you have previously overridden any of")
  #warning ("scBalInit, scBalRecordClear or scBalRecordGenerate")
 #else
  #pragma message ("Consider overriding InitBodyGen if you have previously overridden any of")
  #pragma message ("scBalInit, scBalRecordClear or scBalRecordGenerate")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && (!defined(DO_DELETEBODY) && !defined(DO_DELETEDETAILSBODY)) && (defined(DO_scBalDelete) || defined(DO_scBalFilterDelete))
 #if defined LINUX_VER
  #warning ("Consider overriding either DeleteBody or DeleteDetailsBody if you have previously")
  #warning ("overridden any of scBalDelete or scBalFilterDelete")
 #else
  #pragma message ("Consider overriding either DeleteBody or DeleteDetailsBody if you have previously")
  #pragma message ("overridden any of scBalDelete or scBalFilterDelete")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=0 && ((defined(DO_scBalSecurity) && !defined(DO_scBalInstanceSecurity)) || (!defined(DO_scBalSecurity) && defined(DO_scBalInstanceSecurity)))
 #if defined LINUX_VER
  #warning ("If either of scBalSecurity or scBalInstanceSecurity is overridden")
  #warning ("you should override the other")
 #else
  #pragma message ("If either of scBalSecurity or scBalInstanceSecurity is overridden")
...
// These used to be optional, now are required.
 ...
 #error DO_OLDPOST is no longer supported for non-revisioned views, and results in scBalBasePost doing nothing. You need to remove it, plus change any callers that depended on this behaviour.
#endif
 ...
 #error DO_GETPERMISSIONS is no longer supported. You must override scBalInstanceSecurity instead.
#endif
...
// The following provide simple error checking by view protocol
 ...
 #error View protocol must be explicitly selected
  ...
  #elif KEY_COUNT!=(SCBAL_KEY_COUNT-JOIN_KEY_COUNT+FAKE_KEY_COUNT)
   #error KEY_COUNT must be defined as (SCBAL_KEY_COUNT-JOIN_KEY_COUNT+FAKE_KEY_COUNT)
  #endif
  ...
  #error Under C++ you must derive VEXTRA in scBal3.h from VBASE
 #endif
...
struct tagV {
   ...
   BOOL bVerifying; // inside scBalVerify routine
   ...
   WORD wKey; // Current key as passed to scBalOrder
   ...
   LPVOID OpenExtra; // copy of extra data received by scBalInstanceOpen
   ...
   LONG rotoID; // My own rotoID; needs SCBAL_ROTOID to be defined to be meaningful
...
#if !defined(__cplusplus)
 #if defined(DO_VIEWHANDLE)
   VEXTRA extra; // Programmer-defined view handle extensions, defined in scBal1.h
 #endif
...
// definition in the scBal1.c file must be done.
...
// initialized in the scBal1.c file.
...
/////////////////////////////////////////////////////////////////////////////
// Declarations for generally available subroutines found in scBal.c

// The fileXXXX routines are not always defined by the template. For stub
// views they appear in scBal1.c, and must be specified by the programmer.
...
VIEWCALL scBalFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scBalFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
VIEWCALL scBalFields            (LPVOID lpView, LPSHORT lFields);
VIEWCALL scBalFilterCount       (LPV view, LPSTR filter, A4W_FLAGS flags, LP_A4W_SIZE count);
...
VIEWCALL scBalInstanceFieldList (LPV view, A4W_FLAGS flags, A4W_SIZE count, PLONG fields);
VIEWCALL scBalInstanceNotify    (LPV view, A4W_NOTIFY_PROC notify, LPVOID context, A4W_FLAGS flags);
VIEWCALL scBalInstanceOpen      (LPRVH rvh, HPIB PIB, WORD link, A4W_FLAGS flags, LPVOID extra, A4W_SIZE size, LPV FAR * view);
...
VIEWCALL scBalOpen              (HPIB hPIB, WORD wLinkNo, LPVOID FAR * lpView);
...
VIEWCALL scBalRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
...
VIEWCALL scBalViewTableCount    (INT roto, LPSHORT tables);
VIEWCALL scBalViewTables        (INT roto, LPSTR tables, INT size);
...
VIEWCALL scBalBaseFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scBalBaseFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
...
VIEWCALL scBalBaseRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
...
VIEWCALL scBalExtendedBaseAttribs           (LPVOID lpView, LONG lField, LPLONG lpAttributes);
VIEWCALL scBalExtendedBaseBlkGet            (LPVOID lpView, INT lFields, LPLONG lFieldList, LPSHORT lpnSizeList, LPSTR lpsBuffer);
VIEWCALL scBalExtendedBaseBlkPut            (LPVOID lpView, INT lFields, LPLONG lFieldList, LPSHORT lpnSizeList, LPSTR lpsBuffer, BOOL bVerify);
VIEWCALL scBalExtendedBaseBrowse            (LPVOID lpView, LPSTR lpsExpression, BOOL bAscending);
VIEWCALL scBalExtendedBaseCancel            (LPVOID lpView);
VIEWCALL scBalExtendedBaseClose             (LPVOID lpView);
VIEWCALL scBalExtendedBaseCmpNames          (INT roto, LPSTR lpsCompositeNames, LPSHORT lpnCount);
VIEWCALL scBalExtendedBaseCompose           (LPVOID lpView, LPVIEWDEF lpvdefComposites);
VIEWCALL scBalExtendedBaseCreate            (HPIB hPIB, WORD wLinkNo);
VIEWCALL scBalExtendedBaseDelete            (LPVOID lpView);
VIEWCALL scBalExtendedBaseDirty             (LPVOID lpView, LPBOOL bChanged);
VIEWCALL scBalExtendedBaseDrop              (HPIB hPIB, WORD wLinkNo);
VIEWCALL scBalExtendedBaseExists            (LPVOID lpView, LPBOOL bExists);
VIEWCALL scBalExtendedBaseFetch             (LPVOID lpView);
VIEWCALL scBalExtendedBaseFetchLock         (LPVOID lpView);
VIEWCALL scBalExtendedBaseField             (LPVOID lpView, INT nFldNumber, LPSTR lpsFldName, LPLONG lField);
VIEWCALL scBalExtendedBaseFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scBalExtendedBaseFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
VIEWCALL scBalExtendedBaseFields            (LPVOID lpView, LPSHORT lFields);
VIEWCALL scBalExtendedBaseFilterCount       (LPV view, LPSTR filter, A4W_FLAGS flags, LP_A4W_SIZE count);
VIEWCALL scBalExtendedBaseFilterDelete      (LPV view, LPSTR filter, A4W_FLAGS flags);
VIEWCALL scBalExtendedBaseFilterFetch       (LPV view, A4W_FLAGS flags);
VIEWCALL scBalExtendedBaseFilterSelect      (LPV view, INT key, LPSTR filter, A4W_FLAGS flags);
VIEWCALL scBalExtendedBaseGet               (LPVOID lpView, LONG lField, LPSTR buffer, INT nSize);
VIEWCALL scBalExtendedBaseInit              (LPVOID lpView);
VIEWCALL scBalExtendedBaseInsert            (LPVOID lpView);
...
VIEWCALL scBalExtendedBaseInstanceProtocol  (LPV view, A4W_FLAGS flags, LP_A4W_FLAGS protocols, LP_A4W_SIZE count);
VIEWCALL scBalExtendedBaseInstanceSecurity  (LPV view, LP_A4W_FLAGS flags);
VIEWCALL scBalExtendedBaseKey               (LPVOID lpView, INT nKeyNo, LPSTR sName, LPSHORT lFields, LPLONG lplKeyFields);
VIEWCALL scBalExtendedBaseKeys              (LPVOID lpView, LPSHORT lpnKeys);
VIEWCALL scBalExtendedBaseName              (LPVOID lpView, LONG lField, LPSTR lpsName, LPSTR lpsDesc);
VIEWCALL scBalExtendedBaseOpen              (HPIB hPIB, WORD wLinkNo, LPVOID FAR * lpView);
VIEWCALL scBalExtendedBaseOrder             (LPVOID lpView, INT nKey);
VIEWCALL scBalExtendedBasePost              (LPVOID lpView);
VIEWCALL scBalExtendedBasePresents          (LPVOID lpView, LONG lField, LPSTR lpsMask, LPSHORT lpnList, LPSHORT lpnCount, LPVOID lpBuffer, INT nSize);
VIEWCALL scBalExtendedBaseProcess           (LPVOID lpView);
VIEWCALL scBalExtendedBasePut               (LPVOID lpView, LONG lField, LPSTR lpsBuffer, INT nSize, BOOL bVerify);
VIEWCALL scBalExtendedBaseRead              (LPVOID lpView);
VIEWCALL scBalExtendedBaseReadLock          (LPVOID lpView);
VIEWCALL scBalExtendedBaseRecordClear       (LPV view);
VIEWCALL scBalExtendedBaseRecordGenerate    (LPV view, A4W_FLAGS flags);
VIEWCALL scBalExtendedBaseRevisionCancel    (LPV view, INT level);
VIEWCALL scBalExtendedBaseRevisionExists    (LPV view, INT level, LP_A4W_FLAGS flags);
VIEWCALL scBalExtendedBaseRevisionPost      (LPV view, INT level);
VIEWCALL scBalExtendedBaseRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
VIEWCALL scBalExtendedBaseRevisionUnposted  (LPV view, INT level, LP_A4W_FLAGS flags);
VIEWCALL scBalExtendedBaseSecurity          (HPIB hPIB, WORD wLinkNo, LPLONG lplPermissions);
VIEWCALL scBalExtendedBaseTableEmpty        (LPV view);
VIEWCALL scBalExtendedBaseType              (LPVOID lpView, LONG lField, LPTYPE lptype);
VIEWCALL scBalExtendedBaseUnLock            (LPVOID lpView);
VIEWCALL scBalExtendedBaseUpdate            (LPVOID lpView);
VIEWCALL scBalExtendedBaseVerify            (LPVOID lpView);
...
VIEWCALL viewOldVerify            (LPVOID rvh, LPVOID view);
 ...
 #if defined(__scBal_c) || DO_EXTENDEDERRORS>NO_EXTENDEDERRORS
