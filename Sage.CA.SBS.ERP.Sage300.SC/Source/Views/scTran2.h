...
#if !defined(__scTran2_h)
// Ensures header is only included once.
#define __scTran2_h        1
...
#if TEMPLATE_WARNING_LEVEL<=0 && !defined(SCTRAN_ROTOID)
 #if defined LINUX_VER
  #warning ("The template disables some features when SCTRAN_ROTOID has been removed")
  #warning ("from scTran.h. Please define it to activate these features")
 #else
  #pragma message ("The template disables some features when SCTRAN_ROTOID has been removed")
  #pragma message ("from scTran.h. Please define it to activate these features")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_XXXFETCHFILTER) && (defined(DO_scTranFetch) || defined(DO_scTranFetchLock) || defined(DO_scTranFilterFetch))
 #if defined LINUX_VER
  #warning ("Consider overriding xxxFetchFilter if you have previously overridden any of")
  #warning ("scTranFetch, scTranFetchLock or scTranFilterFetch")
 #else
  #pragma message ("Consider overriding xxxFetchFilter if you have previously overridden any of")
  #pragma message ("scTranFetch, scTranFetchLock or scTranFilterFetch")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_XXXREAD) && (defined(DO_scTranRead) || defined(DO_scTranReadLock))
 #if defined LINUX_VER
  #warning ("Consider overriding xxxRead if you have previously overridden any of")
  #warning ("scTranRead or scTranReadLock")
 #else
  #pragma message ("Consider overriding xxxRead if you have previously overridden any of")
  #pragma message ("scTranRead or scTranReadLock")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_INITBODYGEN) && (defined(DO_scTranInit) || defined(DO_scTranRecordClear) || defined(DO_scTranRecordGenerate))
 #if defined LINUX_VER
  #warning ("Consider overriding InitBodyGen if you have previously overridden any of")
  #warning ("scTranInit, scTranRecordClear or scTranRecordGenerate")
 #else
  #pragma message ("Consider overriding InitBodyGen if you have previously overridden any of")
  #pragma message ("scTranInit, scTranRecordClear or scTranRecordGenerate")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && (!defined(DO_DELETEBODY) && !defined(DO_DELETEDETAILSBODY)) && (defined(DO_scTranDelete) || defined(DO_scTranFilterDelete))
 #if defined LINUX_VER
  #warning ("Consider overriding either DeleteBody or DeleteDetailsBody if you have previously")
  #warning ("overridden any of scTranDelete or scTranFilterDelete")
 #else
  #pragma message ("Consider overriding either DeleteBody or DeleteDetailsBody if you have previously")
  #pragma message ("overridden any of scTranDelete or scTranFilterDelete")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=0 && ((defined(DO_scTranSecurity) && !defined(DO_scTranInstanceSecurity)) || (!defined(DO_scTranSecurity) && defined(DO_scTranInstanceSecurity)))
 #if defined LINUX_VER
  #warning ("If either of scTranSecurity or scTranInstanceSecurity is overridden")
  #warning ("you should override the other")
 #else
  #pragma message ("If either of scTranSecurity or scTranInstanceSecurity is overridden")
...
// These used to be optional, now are required.
 ...
 #error DO_OLDPOST is no longer supported for non-revisioned views, and results in scTranBasePost doing nothing. You need to remove it, plus change any callers that depended on this behaviour.
#endif
 ...
 #error DO_GETPERMISSIONS is no longer supported. You must override scTranInstanceSecurity instead.
#endif
...
// The following provide simple error checking by view protocol
 ...
 #error View protocol must be explicitly selected
  ...
  #elif KEY_COUNT!=(SCTRAN_KEY_COUNT-JOIN_KEY_COUNT+FAKE_KEY_COUNT)
   #error KEY_COUNT must be defined as (SCTRAN_KEY_COUNT-JOIN_KEY_COUNT+FAKE_KEY_COUNT)
  #endif
  ...
  #error Under C++ you must derive VEXTRA in scTran3.h from VBASE
 #endif
...
struct tagV {
   ...
   BOOL bVerifying; // inside scTranVerify routine
   ...
   WORD wKey; // Current key as passed to scTranOrder
   ...
   LPVOID OpenExtra; // copy of extra data received by scTranInstanceOpen
   ...
   LONG rotoID; // My own rotoID; needs SCTRAN_ROTOID to be defined to be meaningful
...
#if !defined(__cplusplus)
 #if defined(DO_VIEWHANDLE)
   VEXTRA extra; // Programmer-defined view handle extensions, defined in scTran1.h
 #endif
...
// definition in the scTran1.c file must be done.
...
// initialized in the scTran1.c file.
...
/////////////////////////////////////////////////////////////////////////////
// Declarations for generally available subroutines found in scTran.c

// The fileXXXX routines are not always defined by the template. For stub
// views they appear in scTran1.c, and must be specified by the programmer.
...
VIEWCALL scTranFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scTranFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
VIEWCALL scTranFields            (LPVOID lpView, LPSHORT lFields);
VIEWCALL scTranFilterCount       (LPV view, LPSTR filter, A4W_FLAGS flags, LP_A4W_SIZE count);
...
VIEWCALL scTranInstanceFieldList (LPV view, A4W_FLAGS flags, A4W_SIZE count, PLONG fields);
VIEWCALL scTranInstanceNotify    (LPV view, A4W_NOTIFY_PROC notify, LPVOID context, A4W_FLAGS flags);
VIEWCALL scTranInstanceOpen      (LPRVH rvh, HPIB PIB, WORD link, A4W_FLAGS flags, LPVOID extra, A4W_SIZE size, LPV FAR * view);
...
VIEWCALL scTranOpen              (HPIB hPIB, WORD wLinkNo, LPVOID FAR * lpView);
...
VIEWCALL scTranRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
...
VIEWCALL scTranViewTableCount    (INT roto, LPSHORT tables);
VIEWCALL scTranViewTables        (INT roto, LPSTR tables, INT size);
...
VIEWCALL scTranBaseFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scTranBaseFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
...
VIEWCALL scTranBaseRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
...
VIEWCALL scTranExtendedBaseAttribs           (LPVOID lpView, LONG lField, LPLONG lpAttributes);
VIEWCALL scTranExtendedBaseBlkGet            (LPVOID lpView, INT lFields, LPLONG lFieldList, LPSHORT lpnSizeList, LPSTR lpsBuffer);
VIEWCALL scTranExtendedBaseBlkPut            (LPVOID lpView, INT lFields, LPLONG lFieldList, LPSHORT lpnSizeList, LPSTR lpsBuffer, BOOL bVerify);
VIEWCALL scTranExtendedBaseBrowse            (LPVOID lpView, LPSTR lpsExpression, BOOL bAscending);
VIEWCALL scTranExtendedBaseCancel            (LPVOID lpView);
VIEWCALL scTranExtendedBaseClose             (LPVOID lpView);
VIEWCALL scTranExtendedBaseCmpNames          (INT roto, LPSTR lpsCompositeNames, LPSHORT lpnCount);
VIEWCALL scTranExtendedBaseCompose           (LPVOID lpView, LPVIEWDEF lpvdefComposites);
VIEWCALL scTranExtendedBaseCreate            (HPIB hPIB, WORD wLinkNo);
VIEWCALL scTranExtendedBaseDelete            (LPVOID lpView);
VIEWCALL scTranExtendedBaseDirty             (LPVOID lpView, LPBOOL bChanged);
VIEWCALL scTranExtendedBaseDrop              (HPIB hPIB, WORD wLinkNo);
VIEWCALL scTranExtendedBaseExists            (LPVOID lpView, LPBOOL bExists);
VIEWCALL scTranExtendedBaseFetch             (LPVOID lpView);
VIEWCALL scTranExtendedBaseFetchLock         (LPVOID lpView);
VIEWCALL scTranExtendedBaseField             (LPVOID lpView, INT nFldNumber, LPSTR lpsFldName, LPLONG lField);
VIEWCALL scTranExtendedBaseFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scTranExtendedBaseFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
VIEWCALL scTranExtendedBaseFields            (LPVOID lpView, LPSHORT lFields);
VIEWCALL scTranExtendedBaseFilterCount       (LPV view, LPSTR filter, A4W_FLAGS flags, LP_A4W_SIZE count);
VIEWCALL scTranExtendedBaseFilterDelete      (LPV view, LPSTR filter, A4W_FLAGS flags);
VIEWCALL scTranExtendedBaseFilterFetch       (LPV view, A4W_FLAGS flags);
VIEWCALL scTranExtendedBaseFilterSelect      (LPV view, INT key, LPSTR filter, A4W_FLAGS flags);
VIEWCALL scTranExtendedBaseGet               (LPVOID lpView, LONG lField, LPSTR buffer, INT nSize);
VIEWCALL scTranExtendedBaseInit              (LPVOID lpView);
VIEWCALL scTranExtendedBaseInsert            (LPVOID lpView);
...
VIEWCALL scTranExtendedBaseInstanceProtocol  (LPV view, A4W_FLAGS flags, LP_A4W_FLAGS protocols, LP_A4W_SIZE count);
VIEWCALL scTranExtendedBaseInstanceSecurity  (LPV view, LP_A4W_FLAGS flags);
VIEWCALL scTranExtendedBaseKey               (LPVOID lpView, INT nKeyNo, LPSTR sName, LPSHORT lFields, LPLONG lplKeyFields);
VIEWCALL scTranExtendedBaseKeys              (LPVOID lpView, LPSHORT lpnKeys);
VIEWCALL scTranExtendedBaseName              (LPVOID lpView, LONG lField, LPSTR lpsName, LPSTR lpsDesc);
VIEWCALL scTranExtendedBaseOpen              (HPIB hPIB, WORD wLinkNo, LPVOID FAR * lpView);
VIEWCALL scTranExtendedBaseOrder             (LPVOID lpView, INT nKey);
VIEWCALL scTranExtendedBasePost              (LPVOID lpView);
VIEWCALL scTranExtendedBasePresents          (LPVOID lpView, LONG lField, LPSTR lpsMask, LPSHORT lpnList, LPSHORT lpnCount, LPVOID lpBuffer, INT nSize);
VIEWCALL scTranExtendedBaseProcess           (LPVOID lpView);
VIEWCALL scTranExtendedBasePut               (LPVOID lpView, LONG lField, LPSTR lpsBuffer, INT nSize, BOOL bVerify);
VIEWCALL scTranExtendedBaseRead              (LPVOID lpView);
VIEWCALL scTranExtendedBaseReadLock          (LPVOID lpView);
VIEWCALL scTranExtendedBaseRecordClear       (LPV view);
VIEWCALL scTranExtendedBaseRecordGenerate    (LPV view, A4W_FLAGS flags);
VIEWCALL scTranExtendedBaseRevisionCancel    (LPV view, INT level);
VIEWCALL scTranExtendedBaseRevisionExists    (LPV view, INT level, LP_A4W_FLAGS flags);
VIEWCALL scTranExtendedBaseRevisionPost      (LPV view, INT level);
VIEWCALL scTranExtendedBaseRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
VIEWCALL scTranExtendedBaseRevisionUnposted  (LPV view, INT level, LP_A4W_FLAGS flags);
VIEWCALL scTranExtendedBaseSecurity          (HPIB hPIB, WORD wLinkNo, LPLONG lplPermissions);
VIEWCALL scTranExtendedBaseTableEmpty        (LPV view);
VIEWCALL scTranExtendedBaseType              (LPVOID lpView, LONG lField, LPTYPE lptype);
VIEWCALL scTranExtendedBaseUnLock            (LPVOID lpView);
VIEWCALL scTranExtendedBaseUpdate            (LPVOID lpView);
VIEWCALL scTranExtendedBaseVerify            (LPVOID lpView);
...
VIEWCALL viewOldVerify            (LPVOID rvh, LPVOID view);
 ...
 #if defined(__scTran_c) || DO_EXTENDEDERRORS>NO_EXTENDEDERRORS
