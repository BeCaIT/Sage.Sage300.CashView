...
#if TEMPLATE_WARNING_LEVEL<=0 && !defined(SCOPT_ROTOID)
 #if defined LINUX_VER
  #warning ("The template disables some features when SCOPT_ROTOID has been removed")
  #warning ("from scOpt.h. Please define it to activate these features")
 #else
  #pragma message ("The template disables some features when SCOPT_ROTOID has been removed")
  #pragma message ("from scOpt.h. Please define it to activate these features")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_XXXFETCHFILTER) && (defined(DO_scOptFetch) || defined(DO_scOptFetchLock) || defined(DO_scOptFilterFetch))
 #if defined LINUX_VER
  #warning ("Consider overriding xxxFetchFilter if you have previously overridden any of")
  #warning ("scOptFetch, scOptFetchLock or scOptFilterFetch")
 #else
  #pragma message ("Consider overriding xxxFetchFilter if you have previously overridden any of")
  #pragma message ("scOptFetch, scOptFetchLock or scOptFilterFetch")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_XXXREAD) && (defined(DO_scOptRead) || defined(DO_scOptReadLock))
 #if defined LINUX_VER
  #warning ("Consider overriding xxxRead if you have previously overridden any of")
  #warning ("scOptRead or scOptReadLock")
 #else
  #pragma message ("Consider overriding xxxRead if you have previously overridden any of")
  #pragma message ("scOptRead or scOptReadLock")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && !defined(DO_INITBODYGEN) && (defined(DO_scOptInit) || defined(DO_scOptRecordClear) || defined(DO_scOptRecordGenerate))
 #if defined LINUX_VER
  #warning ("Consider overriding InitBodyGen if you have previously overridden any of")
  #warning ("scOptInit, scOptRecordClear or scOptRecordGenerate")
 #else
  #pragma message ("Consider overriding InitBodyGen if you have previously overridden any of")
  #pragma message ("scOptInit, scOptRecordClear or scOptRecordGenerate")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=1 && (!defined(DO_DELETEBODY) && !defined(DO_DELETEDETAILSBODY)) && (defined(DO_scOptDelete) || defined(DO_scOptFilterDelete))
 #if defined LINUX_VER
  #warning ("Consider overriding either DeleteBody or DeleteDetailsBody if you have previously")
  #warning ("overridden any of scOptDelete or scOptFilterDelete")
 #else
  #pragma message ("Consider overriding either DeleteBody or DeleteDetailsBody if you have previously")
  #pragma message ("overridden any of scOptDelete or scOptFilterDelete")
 #endif
#endif

#if TEMPLATE_WARNING_LEVEL<=0 && ((defined(DO_scOptSecurity) && !defined(DO_scOptInstanceSecurity)) || (!defined(DO_scOptSecurity) && defined(DO_scOptInstanceSecurity)))
 #if defined LINUX_VER
  #warning ("If either of scOptSecurity or scOptInstanceSecurity is overridden")
  #warning ("you should override the other")
 #else
  #pragma message ("If either of scOptSecurity or scOptInstanceSecurity is overridden")
...
// These used to be optional, now are required.
 ...
 #error DO_OLDPOST is no longer supported for non-revisioned views, and results in scOptBasePost doing nothing. You need to remove it, plus change any callers that depended on this behaviour.
#endif
 ...
 #error DO_GETPERMISSIONS is no longer supported. You must override scOptInstanceSecurity instead.
#endif
...
// The following provide simple error checking by view protocol
 ...
 #error View protocol must be explicitly selected
  ...
  #elif KEY_COUNT!=(SCOPT_KEY_COUNT-JOIN_KEY_COUNT+FAKE_KEY_COUNT)
   #error KEY_COUNT must be defined as (SCOPT_KEY_COUNT-JOIN_KEY_COUNT+FAKE_KEY_COUNT)
  #endif
  ...
  #error Under C++ you must derive VEXTRA in scOpt3.h from VBASE
 #endif
...
struct tagV {
   ...
   BOOL bVerifying; // inside scOptVerify routine
   ...
   WORD wKey; // Current key as passed to scOptOrder
   ...
   LPVOID OpenExtra; // copy of extra data received by scOptInstanceOpen
   ...
   LONG rotoID; // My own rotoID; needs SCOPT_ROTOID to be defined to be meaningful
...
#if !defined(__cplusplus)
 #if defined(DO_VIEWHANDLE)
   VEXTRA extra; // Programmer-defined view handle extensions, defined in scOpt1.h
 #endif
...
// definition in the scOpt1.c file must be done.
...
// initialized in the scOpt1.c file.
...
/////////////////////////////////////////////////////////////////////////////
// Declarations for generally available subroutines found in scOpt.c

// The fileXXXX routines are not always defined by the template. For stub
// views they appear in scOpt1.c, and must be specified by the programmer.
...
VIEWCALL scOptFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scOptFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
VIEWCALL scOptFields            (LPVOID lpView, LPSHORT lFields);
VIEWCALL scOptFilterCount       (LPV view, LPSTR filter, A4W_FLAGS flags, LP_A4W_SIZE count);
...
VIEWCALL scOptInstanceFieldList (LPV view, A4W_FLAGS flags, A4W_SIZE count, PLONG fields);
VIEWCALL scOptInstanceNotify    (LPV view, A4W_NOTIFY_PROC notify, LPVOID context, A4W_FLAGS flags);
VIEWCALL scOptInstanceOpen      (LPRVH rvh, HPIB PIB, WORD link, A4W_FLAGS flags, LPVOID extra, A4W_SIZE size, LPV FAR * view);
...
VIEWCALL scOptOpen              (HPIB hPIB, WORD wLinkNo, LPVOID FAR * lpView);
...
VIEWCALL scOptRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
...
VIEWCALL scOptViewTableCount    (INT roto, LPSHORT tables);
VIEWCALL scOptViewTables        (INT roto, LPSTR tables, INT size);
...
VIEWCALL scOptBaseFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scOptBaseFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
...
VIEWCALL scOptBaseRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
...
VIEWCALL scOptExtendedBaseAttribs           (LPVOID lpView, LONG lField, LPLONG lpAttributes);
VIEWCALL scOptExtendedBaseBlkGet            (LPVOID lpView, INT lFields, LPLONG lFieldList, LPSHORT lpnSizeList, LPSTR lpsBuffer);
VIEWCALL scOptExtendedBaseBlkPut            (LPVOID lpView, INT lFields, LPLONG lFieldList, LPSHORT lpnSizeList, LPSTR lpsBuffer, BOOL bVerify);
VIEWCALL scOptExtendedBaseBrowse            (LPVOID lpView, LPSTR lpsExpression, BOOL bAscending);
VIEWCALL scOptExtendedBaseCancel            (LPVOID lpView);
VIEWCALL scOptExtendedBaseClose             (LPVOID lpView);
VIEWCALL scOptExtendedBaseCmpNames          (INT roto, LPSTR lpsCompositeNames, LPSHORT lpnCount);
VIEWCALL scOptExtendedBaseCompose           (LPVOID lpView, LPVIEWDEF lpvdefComposites);
VIEWCALL scOptExtendedBaseCreate            (HPIB hPIB, WORD wLinkNo);
VIEWCALL scOptExtendedBaseDelete            (LPVOID lpView);
VIEWCALL scOptExtendedBaseDirty             (LPVOID lpView, LPBOOL bChanged);
VIEWCALL scOptExtendedBaseDrop              (HPIB hPIB, WORD wLinkNo);
VIEWCALL scOptExtendedBaseExists            (LPVOID lpView, LPBOOL bExists);
VIEWCALL scOptExtendedBaseFetch             (LPVOID lpView);
VIEWCALL scOptExtendedBaseFetchLock         (LPVOID lpView);
VIEWCALL scOptExtendedBaseField             (LPVOID lpView, INT nFldNumber, LPSTR lpsFldName, LPLONG lField);
VIEWCALL scOptExtendedBaseFieldExists       (LPV view, LONG field, LP_A4W_FLAGS flags);
VIEWCALL scOptExtendedBaseFieldName         (LPV view, LONG field, LPSTR name, LPSTR desc, A4W_SIZE size);
VIEWCALL scOptExtendedBaseFields            (LPVOID lpView, LPSHORT lFields);
VIEWCALL scOptExtendedBaseFilterCount       (LPV view, LPSTR filter, A4W_FLAGS flags, LP_A4W_SIZE count);
VIEWCALL scOptExtendedBaseFilterDelete      (LPV view, LPSTR filter, A4W_FLAGS flags);
VIEWCALL scOptExtendedBaseFilterFetch       (LPV view, A4W_FLAGS flags);
VIEWCALL scOptExtendedBaseFilterSelect      (LPV view, INT key, LPSTR filter, A4W_FLAGS flags);
VIEWCALL scOptExtendedBaseGet               (LPVOID lpView, LONG lField, LPSTR buffer, INT nSize);
VIEWCALL scOptExtendedBaseInit              (LPVOID lpView);
VIEWCALL scOptExtendedBaseInsert            (LPVOID lpView);
...
VIEWCALL scOptExtendedBaseInstanceProtocol  (LPV view, A4W_FLAGS flags, LP_A4W_FLAGS protocols, LP_A4W_SIZE count);
VIEWCALL scOptExtendedBaseInstanceSecurity  (LPV view, LP_A4W_FLAGS flags);
VIEWCALL scOptExtendedBaseKey               (LPVOID lpView, INT nKeyNo, LPSTR sName, LPSHORT lFields, LPLONG lplKeyFields);
VIEWCALL scOptExtendedBaseKeys              (LPVOID lpView, LPSHORT lpnKeys);
VIEWCALL scOptExtendedBaseName              (LPVOID lpView, LONG lField, LPSTR lpsName, LPSTR lpsDesc);
VIEWCALL scOptExtendedBaseOpen              (HPIB hPIB, WORD wLinkNo, LPVOID FAR * lpView);
VIEWCALL scOptExtendedBaseOrder             (LPVOID lpView, INT nKey);
VIEWCALL scOptExtendedBasePost              (LPVOID lpView);
VIEWCALL scOptExtendedBasePresents          (LPVOID lpView, LONG lField, LPSTR lpsMask, LPSHORT lpnList, LPSHORT lpnCount, LPVOID lpBuffer, INT nSize);
VIEWCALL scOptExtendedBaseProcess           (LPVOID lpView);
VIEWCALL scOptExtendedBasePut               (LPVOID lpView, LONG lField, LPSTR lpsBuffer, INT nSize, BOOL bVerify);
VIEWCALL scOptExtendedBaseRead              (LPVOID lpView);
VIEWCALL scOptExtendedBaseReadLock          (LPVOID lpView);
VIEWCALL scOptExtendedBaseRecordClear       (LPV view);
VIEWCALL scOptExtendedBaseRecordGenerate    (LPV view, A4W_FLAGS flags);
VIEWCALL scOptExtendedBaseRevisionCancel    (LPV view, INT level);
VIEWCALL scOptExtendedBaseRevisionExists    (LPV view, INT level, LP_A4W_FLAGS flags);
VIEWCALL scOptExtendedBaseRevisionPost      (LPV view, INT level);
VIEWCALL scOptExtendedBaseRevisionRekey     (LPV lpv, LPVOID OldKey, LPVOID NewKey, INT size, A4W_FLAGS flags);
VIEWCALL scOptExtendedBaseRevisionUnposted  (LPV view, INT level, LP_A4W_FLAGS flags);
VIEWCALL scOptExtendedBaseSecurity          (HPIB hPIB, WORD wLinkNo, LPLONG lplPermissions);
VIEWCALL scOptExtendedBaseTableEmpty        (LPV view);
VIEWCALL scOptExtendedBaseType              (LPVOID lpView, LONG lField, LPTYPE lptype);
VIEWCALL scOptExtendedBaseUnLock            (LPVOID lpView);
VIEWCALL scOptExtendedBaseUpdate            (LPVOID lpView);
VIEWCALL scOptExtendedBaseVerify            (LPVOID lpView);
...
VIEWCALL viewOldVerify            (LPVOID rvh, LPVOID view);
 ...
 #if defined(__scOpt_c) || DO_EXTENDEDERRORS>NO_EXTENDEDERRORS
